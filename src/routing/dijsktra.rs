...
impl<'a> Dijkstra<'a> {
    ...
    pub fn dijkstra(&self, from_node_id: u32, to_node_id: u32) -> (Vec<Option<u32>>, u32) {
        let mut queue = BucketQueue::new(60_000);

        let mut edge_from_predecessor = vec![None; self.graph.nodes.len()];
